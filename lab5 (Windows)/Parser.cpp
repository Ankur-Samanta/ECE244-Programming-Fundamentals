...
Shape* parseRectangle(stringstream& line) {
    string name;
    float xcent;
    float ycent;
    float width;
...
Shape* parseTriangleCommand(stringstream& line) {
    ...
    float xthree;
    float ythree;
    line >> name >> xone >> yone >> xtwo >> ytwo >> xthree >> ythree;
    ...
    // Create the shape object and return a pointer to it
    xcent = (xone + xtwo + xthree) / 3;
    ycent = (yone + ytwo + ythree) / 3;
    Shape* myShape = (Shape*) new Triangle(name, xcent, ycent, xone, yone, xtwo, ytwo, xthree, ythree);
